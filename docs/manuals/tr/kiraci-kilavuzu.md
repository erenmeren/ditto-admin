...
- **Owner (sahip)** ve **Admin (yönetici)** rolündeki kullanıcılar kiracıyı
  **yönetebilir** — mağaza, cihaz, üye gibi kaynaklar üzerinde işlem
  yapabilirler.
- **Member (üye)** rolündeki kullanıcılar **salt-okunur (read-only)**
  erişime sahiptir; yönetimsel işlem yapamazlar.

Yeni bir kullanıcı davet edildiğinde, davet yalnızca **Admin** veya
**Member** rolüyle verilebilir; **Owner** rolü davet yoluyla
atanamaz — bu rol korunur.
...
1. Tarayıcınızda uygulamanın giriş sayfasına, yani **`/login`** adresine
   gidin.
2. **Email** alanına hesabınızın e-posta adresini girin.
3. **Password** alanına şifrenizi girin.
4. **Sign in** düğmesine tıklayın.
...
### Sol menü — "Workspace" grubu

Giriş yaptıktan sonra sol taraftaki gezinme menüsünde **"Workspace"** adlı
bir grup görürsünüz. Bu grup, aşağıdaki sırayla şu ekranları içerir:
...
Üst kısımda, aktif organizasyonunuzun adını ve **"Tenant Workspace"**
etiketini gösteren bir **çalışma alanı değiştirici (workspace switcher)**
bulunur. Bu kontrole tıkladığınızda açılan menüde:
...
- Listeden başka bir organizasyon seçildiğinde, `setActive()` çağrısıyla o
  ...
  yönlendirilirsiniz.
- Hesabınız aynı zamanda **platform_admin** ise, menüde ayrıca **"Ditto HQ /
  Super Admin"** adlı bir seçenek görünür; bu seçenek sizi **`/admin`**
  adresine götürür.

### Hesap menüsü

Üst çubuktaki hesap menüsünü açtığınızda, kullanıcının **avatarı, adı ve
rolü** görüntülenir. Menüde ayrıca şu öğeler bulunur: **Profile**,
**Settings**, bir **tema değiştirici (ThemeToggle)** ve **Sign out**.

> **Önemli — işlevsiz menü öğeleri:** Hesap menüsündeki **Profile** ve
> **Settings** öğeleri **işlevsizdir** — bu öğelere tıklamanın herhangi bir
> etkisi yoktur, herhangi bir sayfa açılmaz. **Sign out** öğesi ise gerçek
> bir oturum kapatma işlemi **çağırmaz**; yalnızca sizi **`/login`**
> sayfasına yönlendirir. Bu nedenle "Sign out" öğesine tıkladıktan sonra
> tarayıcı geçmişinden geri gitmeniz veya sayfayı yenilemeniz durumunda,
> oturumunuzun teknik olarak hâlâ açık olabileceğini unutmayın.
...
- Hesap menüsündeki **Profile**, **Settings** ve **Sign out** öğelerinin
  yukarıda açıklanan sınırlamalarını unutmayın; gerçek bir oturum kapatma
  için tarayıcınızın kendi araçlarına (örn. çerezleri temizleme) ihtiyaç
  duyabilirsiniz.
